...
module ``Data 2016`` =
    ...
    let germany = { Name = "Germany"; Status = Eliminated; Seeding = 1; Coach = "Joachim Löw" }
    let ukraine = { Name = "Ukraine"; Status = Eliminated; Seeding = 12; Coach = "Mykhaylo Fomenko" }
    let poland = { Name = "Poland"; Status = Eliminated; Seeding = 15; Coach = "Adam Nawałka" }
    let northernIreland = { Name = "Northern Ireland"; Status = Eliminated; Seeding = 24; Coach = "Michael O'Neill" }

    // Group D.

    let spain = { Name = "Spain"; Status = Eliminated; Seeding = 2; Coach = "Vicente del Bosque" }
    let croatia = { Name = "Croatia"; Status = Eliminated; Seeding = 11; Coach = "Ante Čačić" }
    let czechRepublic = { Name = "Czech Republic"; Status = Eliminated; Seeding = 13; Coach = "Pavel Vrba" }
    let turkey = { Name = "Turkey"; Status = Eliminated; Seeding = 19; Coach = "Fatih Terim" }

    // Group E.

    let belgium = { Name = "Belgium"; Status = Eliminated; Seeding = 5; Coach = "Marc Wilmots" }
    let italy = { Name = "Italy"; Status = Eliminated; Seeding = 6; Coach = "Antonio Conte" }
    ...
    let groupEItalyVsRepublicOfIreland = { Team1Score = TeamMatchScore (italy, 0<goal>, None); Team2Score = TeamMatchScore (republicOfIreland, 1<goal>, None)
                                           Number = 35; Stage = groupE; KickOff = DateTime (2016, 06, 22, 20, 00, 00)
                                           Events = [ ManOfTheMatch bradyRobbie
                                                      YellowCard (siriguSalvatore, Some 39<min>) (* italy *)
                                                      YellowCard (barzagliAndrea, Some 79<min>)
                                                      YellowCard (zazaSimone, Some 87<min>)
                                                      YellowCard (insigneLorenzo, Some 92<min>)
                                                      YellowCard (longShane, Some 39<min>) (* republicOfIreland *)
                                                      YellowCard (wardStephen, Some 73<min>)
                                                      Goal (bradyRobbie, Some 85<min>, Some hoolahanWes)
                                                      CleanSheet (randolphDarren, None) ] }
                                           
    let groupESwedenVsBelgium = { Team1Score = TeamMatchScore (sweden, 0<goal>, None); Team2Score = TeamMatchScore (belgium, 1<goal>, None)
                                  Number = 36; Stage = groupE; KickOff = DateTime (2016, 06, 22, 20, 00, 00)
                                  Events = [ ManOfTheMatch hazardEden
                                             YellowCard (ekdalAlbin, Some 33<min>) (* sweden *)
                                             YellowCard (johanssonErik, Some 36<min>)
                                             YellowCard (meunierThomas, Some 30<min>) (* belgium *)
                                             YellowCard (witselAxel, Some 46<min>)
                                             Goal (nainggolanRadja, Some 84<min>, Some hazardEden)
                                             CleanSheet (courtoisThibaut, None) ] }
    ...
    let groupFIcelandVsAustria = { Team1Score = TeamMatchScore (iceland, 2<goal>, None); Team2Score = TeamMatchScore (austria, 1<goal>, None)
                                   Number = 33; Stage = groupF; KickOff = DateTime (2016, 06, 22, 17, 00, 00)
                                   Events = [ ManOfTheMatch arnasonKari
                                              Goal (bodvarssonJonDadi, Some 18<min>, Some arnasonKari) (* iceland *)
                                              YellowCard (skulasonAriFreyr, Some 36<min>)
                                              YellowCard (sigthorssonKolbeinn, Some 51<min>)
                                              YellowCard (arnasonKari, Some 78<min>)
                                              YellowCard (halldorssonHannesThor, Some 82<min>)
                                              Goal (traustasonArnorIngvi, Some 94<min>, Some bjarnssonTheodorElmar)
                                              Penalty (dragovicAleksandar, false, Some 36<min>, Some alabaDavid, None) (* austria *)
                                              Goal (schopfAlessandro, Some 60<min>, Some alabaDavid)
                                              YellowCard (jankoMarc, Some 70<min>) ] }
                                           
    let groupFHungaryVsPortugal = { Team1Score = TeamMatchScore (hungary, 3<goal>, None); Team2Score = TeamMatchScore (portugal, 3<goal>, None)
                                    Number = 34; Stage = groupF; KickOff = DateTime (2016, 06, 22, 17, 00, 00)
                                    Events = [ ManOfTheMatch ronaldoCristiano
                                               YellowCard (guzmicsRichard, Some 13<min>) (* hungary *)
                                               Goal (geraZoltan, Some 19<min>, None)
                                               YellowCard (juhaszRoland, Some 28<min>)
                                               YellowCard (geraZoltan, Some 34<min>)
                                               Goal (dzsudzsakBalazs, Some 47<min>, Some szalaiAdam)
                                               Goal (dzsudzsakBalazs, Some 55<min>, None)
                                               YellowCard (dzsudzsakBalazs, Some 56<min>)
                                               Goal (nani, Some 42<min>, Some ronaldoCristiano) (* portugal *)
                                               Goal (ronaldoCristiano, Some 50<min>, Some marioJoao)
                                               Goal (ronaldoCristiano, Some 62<min>, Some quaresmaRicardo) ] }
    ...
    let match37SwitzerlandVsPoland = { Team1Score = TeamMatchScore (switzerland, 1<goal>, Some 4<shootoutPenalty>); Team2Score = TeamMatchScore (poland, 1<goal>, Some 5<shootoutPenalty>)
                                       ...
                                       Events = [ ManOfTheMatch shaqiriXherdan
                                                  YellowCard (scharFabian, Some 55<min>) (* switzerland *)
                                                  Goal (shaqiriXherdan, Some 82<min>, Some derdiyokEren)
                                                  YellowCard (djourouJohan, Some 117<min>)
                                                  YellowCard (jedrzejczykArtur, Some 58<min>) (* poland *)
                                                  Goal (blaszczykowskiJakub, Some 39<min>, Some grosickiKamil)
                                                  YellowCard (pazdanMichal, Some 111<min>) ] }                                                 
                                           
    let match38WalesVsNorthernIreland = { Team1Score = TeamMatchScore (wales, 1<goal>, None); Team2Score = TeamMatchScore (northernIreland, 0<goal>, None)
                                          Number = 38; Stage = RoundOf16 38; KickOff = DateTime (2016, 06, 25, 17, 00, 00)
                                          Events = [ ManOfTheMatch baleGareth
                                                     YellowCard (taylorNeil, Some 58<min>) (* wales *)
                                                     YellowCard (ramseyAaron, Some 94<min>)
                                                     CleanSheet (hennesseyWayne, None)
                                                     YellowCard (dallasStuart, Some 44<min>) (* northernIreland *)                                                     
                                                     YellowCard (davisSteven, Some 67<min>)
                                                     OwnGoal (wales, mcAuleyGareth, Some 75<min>, Some baleGareth) ] }

    let match39CroatiaVsPortugal = { Team1Score = TeamMatchScore (croatia, 0<goal>, None); Team2Score = TeamMatchScore (portugal, 1<goal>, None)
                                     Number = 39; Stage = RoundOf16 39; KickOff = DateTime (2016, 06, 25, 20, 00, 00)
                                     Events = [ ManOfTheMatch sanchesRenato
                                                (* croatia *)
                                                YellowCard (carvalhoRicardo, Some 78<min>) (* portugal *)
                                                Goal (quaresmaRicardo, Some 117<min>, None)
                                                CleanSheet (patricioRui, None) ] }

    let match40FranceVsRepublicOfIreland = { Team1Score = TeamMatchScore (france, 2<goal>, None); Team2Score = TeamMatchScore (republicOfIreland, 1<goal>, None)
                                             Number = 40; Stage = RoundOf16 40; KickOff = DateTime (2016, 06, 26, 14, 00, 00)
                                             Events = [ ManOfTheMatch griezmannAntoine
                                                        YellowCard (kanteNGolo, Some 27<min>) (* france *)
                                                        YellowCard (ramiAdil, Some 44<min>)
                                                        Goal (griezmannAntoine, Some 58<min>, Some sagnaBacary)
                                                        Goal (griezmannAntoine, Some 61<min>, Some giroudOlivier)
                                                        Penalty (bradyRobbie, true, Some 2<min>, Some longShane, None) (* republicOfIreland *)
                                                        YellowCard (colemanSeamus, Some 25<min>)
                                                        YellowCard (hendrickJeff, Some 41<min>)
                                                        RedCard (duffyShane, Some 66<min>)
                                                        YellowCard (longShane, Some 72<min>) ] }

    let match41GermanyVsSlovakia = { Team1Score = TeamMatchScore (germany, 3<goal>, None); Team2Score = TeamMatchScore (slovakia, 0<goal>, None)
                                     Number = 41; Stage = RoundOf16 41; KickOff = DateTime (2016, 06, 26, 17, 00, 00)
                                     Events = [ ManOfTheMatch draxlerJulian
                                                Goal (boatengJerome, Some 8<min>, None) (* germany *)
                                                Penalty (ozilMesut, false, Some 12<min>, Some gomezMario, Some kozacikMatus)
                                                Goal (gomezMario, Some 43<min>, Some draxlerJulian)
                                                YellowCard (kimmichJoshua, Some 46<min>)
                                                Goal (draxlerJulian, Some 63<min>, Some hummelsMatt)
                                                YellowCard (hummelsMatt, Some 67<min>)
                                                CleanSheet (neuerManuel, None)
                                                YellowCard (skrtelMartin, Some 13<min>) (* slovakia *)
                                                YellowCard (kuckaJuraj, Some 91<min>) ] }

    let match42HungaryVsBelgium = { Team1Score = TeamMatchScore (hungary, 0<goal>, None); Team2Score = TeamMatchScore (belgium, 4<goal>, None)
                                    Number = 42; Stage = RoundOf16 42; KickOff = DateTime (2016, 06, 26, 20, 00, 00)
                                    Events = [ ManOfTheMatch hazardEden
                                               YellowCard (kadarTamas, Some 34<min>) (* hungary *)
                                               YellowCard (langAdam, Some 47<min>)
                                               YellowCard (elekAkos, Some 61<min>)
                                               YellowCard (szalaiAdam, Some 92<min>)
                                               Goal (alderweireldToby, Some 10<min>, Some deBruyneKevin) (* belgium *)
                                               YellowCard (vermaelenThomas, Some 68<min>)
                                               Goal (batshuayiMichy, Some 78<min>, Some hazardEden)
                                               Goal (hazardEden, Some 80<min>, Some deBruyneKevin)
                                               YellowCard (batshuayiMichy, Some 89<min>)
                                               Goal (carrascoYannickFerreira, Some 91<min>, Some nainggolanRadja)
                                               YellowCard (fellainiMarouane, Some 92<min>)
                                               CleanSheet (courtoisThibaut, None) ] }
                         
    let match43ItalyVsSpain = { Team1Score = TeamMatchScore (italy, 2<goal>, None); Team2Score = TeamMatchScore (spain, 0<goal>, None)
                                Number = 43; Stage = RoundOf16 43; KickOff = DateTime (2016, 06, 27, 17, 00, 00)
                                Events = [ ManOfTheMatch bonucciLeonardo
                                           YellowCard (deSciglioMattia, Some 24<min>) (* italy *)
                                           Goal (chielliniGiorgio, Some 33<min>, Some giaccheriniEmanuele)
                                           YellowCard (pelleGraciano, Some 54<min>)
                                           YellowCard (mottaThiago, Some 89<min>)
                                           Goal (pelleGraciano, Some 91<min>, None)
                                           CleanSheet (buffonGianluigi, None)
                                           YellowCard (nolito, Some 41<min>) (* spain *)
                                           YellowCard (busquetsSergio, Some 89<min>)
                                           YellowCard (silvaDavid, Some 94<min>) ] }

    let match44EnglandVsIceland = { Team1Score = TeamMatchScore (england, 1<goal>, None); Team2Score = TeamMatchScore (iceland, 2<goal>, None)
                                    Number = 44; Stage = RoundOf16 44; KickOff = DateTime (2016, 06, 27, 20, 00, 00)
                                    Events = [ ManOfTheMatch sigurdssonRagnar
                                               Penalty (rooneyWayne, true, Some 4<min>, Some sterlingRaheem, None) (* england *)
                                               YellowCard (sturridgeDaniel, Some 47<min>)
                                               Goal (sigurdssonRagnar, Some 6<min>, Some arnasonKari) (* iceland *)
                                               Goal (sigthorssonKolbeinn, Some 18<min>, Some bodvarssonJonDadi)
                                               YellowCard (sigurdssonGylfi, Some 38<min>)
                                               YellowCard (gunnarssonAron, Some 65<min>) ] }

    let quarterFinal1PolandVsPortugal = { Team1Score = TeamMatchScore (poland, 1<goal>, Some 3<shootoutPenalty>); Team2Score = TeamMatchScore (portugal, 1<goal>, Some 5<shootoutPenalty>)
                                          Number = 45; Stage = QuarterFinal 1; KickOff = DateTime (2016, 06, 30, 20, 00, 00)
                                          Events = [ ManOfTheMatch sanchesRenato
                                                     Goal (lewandowskiRobert, Some 2<min>, Some grosickiKamil) (* poland *)
                                                     YellowCard (jedrzejczykArtur, Some 42<min>)
                                                     YellowCard (glikKamil, Some 66<min>)
                                                     YellowCard (kapustkaBartosz, Some 89<min>)
                                                     Goal (sanchesRenato, Some 33<min>, Some nani) (* portugal *)
    ...
    let quarterFinal3GermanyVsItaly = { Team1Score = TeamMatchScore (germany, 1<goal>, Some 6<shootoutPenalty>); Team2Score = TeamMatchScore (italy, 1<goal>, Some 5<shootoutPenalty>)
                                        Number = 47; Stage = QuarterFinal 3; KickOff = DateTime (2016, 07, 02, 20, 00, 00)
                                        Events = [ ManOfTheMatch neuerManuel
                                                   Goal (ozilMesut, Some 65<min>, None) (* germany *)
                                                   YellowCard (hummelsMatt, Some 90<min>)
                                                   YellowCard (schweinsteigerBastian, Some 112<min>)                                                   
                                                   YellowCard (sturaroStefano, Some 56<min>) (* italy *)
                                                   YellowCard (deSciglioMattia, Some 57<min>)
                                                   YellowCard (paroloMarco, Some 59<min>)
                                                   Penalty (bonucciLeonardo, true, Some 78<min>, None, None)
    ...
    let semiFinal2GermanyVsFrance = { Team1Score = TeamMatchScore (germany, 0<goal>, None); Team2Score = TeamMatchScore (france, 2<goal>, None)
                                      Number = 50; Stage = SemiFinal 2; KickOff = DateTime (2016, 07, 07, 20, 00, 00)
                                      Events = [ ManOfTheMatch griezmannAntoine
                                                 YellowCard (canEmre, Some 36<min>) (* germany *)
                                                 YellowCard (schweinsteigerBastian, Some 46<min>)
                                                 YellowCard (ozilMesut, Some 46<min>)
                                                 YellowCard (draxlerJulian, Some 50<min>)
                                                 YellowCard (evraPatrice, Some 43<min>) (* france *)
                                                 Penalty (griezmannAntoine, true, Some 47<min>, None, None)
                                                 Goal (griezmannAntoine, Some 72<min>, None)
